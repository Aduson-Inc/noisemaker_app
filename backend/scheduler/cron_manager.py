...
class CronManager:
    ...
    def __init__(
        self,
        base_script_dir: str = "/opt/noisemaker/scripts",
        base_log_dir: str = "/var/log/noisemaker",
    ):
        """
        ...
        self.cron_comment_prefix = "# NOISEMAKER_USER"
        ...
        logger.info(
            f"Cron manager initialized - scripts: {base_script_dir}, logs: {base_log_dir}"
        )

    def create_user_cron_job(
        self, user_id: str, timezone_str: str, user_env_vars: Dict[str, str]
    ) -> bool:
        """
    ...
    def _create_user_script(
        self, user_id: str, user_env_vars: Dict[str, str]
    ) -> Optional[str]:
        """
        Create executable script for user with environment variables and error handling.
        ...
        try:
            ...
            script_content = self._generate_script_content(
                user_id, user_env_vars, log_path
            )
    ...
    def _generate_script_content(
        self, user_id: str, user_env_vars: Dict[str, str], log_path: str
    ) -> str:
        """
        ...
        main_script_path = "/opt/noisemaker/main.py"  # Path to main execution script

        script_lines = [
            "#!/bin/bash",
            "",
            f"# Noisemaker Engine - User {user_id} Daily Execution",
    ...
    def _add_cron_job(self, user_id: str, cron_time: str, script_path: str) -> bool:
        ...
        try:
            ...
            try:
                result = subprocess.run(
                    ["crontab", temp_cron_file], capture_output=True, text=True
                )
    ...
    def remove_user_cron_job(self, user_id: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                if success:
                    logger.info(
                        f"Removed {removed_count} cron job(s) for user {user_id}"
                    )
                else:
    ...
    def list_user_cron_jobs(self) -> List[Dict[str, str]]:
        """
        List all Noisemaker user cron jobs.
                ...
                if self.cron_comment_prefix in line:
                    ...
                    if len(parts) >= 2:
                        ...
                        if comment.startswith(self.cron_comment_prefix + "_"):
                            user_id = comment.replace(
                                self.cron_comment_prefix + "_", ""
                            )
    ...
    def cleanup_user_files(self, user_id: str) -> bool:
        ...
        try:
            ...
            if cleaned_files:
                logger.info(
                    f"Cleaned up {', '.join(cleaned_files)} files for user {user_id}"
                )
